...
const FooterLinks = ({ heading, items }) => (
  <div className="flex-1 justify-start items-start">
    ...
    {items.map((item, index) => (
      <p key={index} className="font-poppins dark:text-white text-nft-black-1 font-normal text-base cursor-pointer dark:hover:text-nft-gray-2 hover:text-nft-black-2 my-3">{item}</p>
    ))}
  </div>
);
...
const Footer = () => {
  ...
  return (
    <footer className="flexCenter flex-col border-t dark:border-nft-black-1 border-nft-gray-1 sm:py-8 py-16">
      {/* Logo and emil subscription */}
      <div className="w-full minmd:w-4/5 flex flex-row md:flex-col sm:px-4 px-16">
        <div className="flexStart flex-1 flex-col">
          <div className="flexCenter cursor-pointer">
            <Image src={images.logo02} className="contain" width={32} height={32} alt="logo" />
            <p className="dark:text-white text-nft-black-1 font-semibold text-lg ml-1">CryptoKet</p>
          </div>
          <p className="font-poppins dark:text-white text-nft-black-1 font-semibold tet-base mt-6">Get the latest updates</p>
        ...
        <div className="flexBetween flex-row w-full minmd:w-4/5 sm:flex-col mt-7">
          ...
          <div className="flex flex-row sm:mt-4">
            {[images.instagram, images.twitter, images.telegram, images.discord].map((image, index) => (
              <div className="mx-2 cursort-pointer" key={index}>
                <Image src={image} alt="social" styles="contain" with={24} height={24} className={theme === 'light' ? 'filter invert' : undefined} />
              </div>
            ))}
          </div>
        </div>
