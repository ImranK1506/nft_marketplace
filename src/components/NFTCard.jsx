...
const NftCard = ({ nft }) => (
  <Link href={{ pathname: '/nft-details', query: nft }}>
    ...
    xs:mx-w-none sm:w-full sm:min-w-155 minmd:min-w-256 minlg:min-w-327
    dark:bg-nft-black-3 bg-white rounded-2xl p-4 m-4 minlg:m-8 sm:my-2 sm:mx-2 cursor-pointer shadow-md"
    >
      <div className="relative w-full h-52 sm:h-36 xs:56 minmd:h-60 minlg:h-300 rounderd-2xl overflow-hidden">
        <Image src={nft.image || images[`nft${nft.i}`]} layout="fill" styles="cover" alt={`nft${nft.i}`} />
      </div>
      {/* Card content */}
